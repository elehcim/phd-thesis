...
Gas is injected from the open ``front" side of the box, which always points in the dwarf's direction of motion. Its density and temperature vary with position, as discussed below.
...
and its norm as:
\begin{equation}
 |\vect q| = \sqrt{q_0^2+q_1^2+q_2^2+q_3^2}.
\end{equation}
...
\end{equation}
From \eqref{eq:conjugate} follows that $\vect q \bar{\vect q} = |\vect q|^2 \equiv (|\vect q|^2, \vec 0)$. For unit quaternions ($|\vect q| = 1$), we can write $\bar{\vect q} = \vect q^{-1}$.

Unit quaternions are strictly linked to rotations.
They can be used to completely describe a rotation around axis $\vec q$ of an angle $\varphi=q_0/2$.

Let's consider a unit quaternion $\vect q$. It can be always written as:
\begin{equation}
 \vect q = (q_0, \vec q) = (\cos \frac \varphi 2, \sin \frac \varphi 2\, \hat n), \quad \text{with } |\hat n|=1.
\end{equation}

If we consider a vector $\vec x \in \mathbb R^3$ as a quaternion with zero scalar part, from \eqref{eq:quaternion_multiplication} we can observe that the multiplication 
\begin{equation}
\bar{\vect q}\, \vec x \, \vect q 
\end{equation}
is the formula for a rotation for $\vec x$ by an angle $\frac \varphi 2$ around the axis $\hat n$ \citep[sec. 1.4]{Graf2008}.
...
\caption{Adhoc acceleration relative to the total gravitational accelaration of the simulation ID 68 with pericenter 150 kpc.}
\label{fig:adhoc}
\end{figure}
