...
\end{equation}
It is useful to define the averaged (\emph{bulk}) velocity $\vect u$ as:
\begin{equation}
  \vect u(\vect x, t) \equiv \mean{\vect v}(\vect x, t) = \int{\vect v(\vect x, \vect v, t)\, m_p\, f(\vect x, \vect v, t) \dd[3] \vect v}
\end{equation}
For each elementary spatial volume $\dd[3] \vect x$ we can write the local velocity as the sum: $\vect v = \vect u + \vect w$, where $\vect u$ is the average particle velocity and $\vect w$ a term corresponding to the random movement of the particle with respect to this average flow velocity.
