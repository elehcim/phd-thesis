...
\begin{figure}[ht]
  ...
  \caption{\Hi{} map for a snapshot of simulation 41 at the beginning of the infall.
...
To compute the \Hi{} radius, we fit an ellipse to the $1$~\Msun{}~pc$^{-2}$ contour of the \Hi{} map (as shown in Figure~\ref{fig:hi_ellipse}).
$r_{\text{\Hi}}$ is thus defined as its major axis.
...
In case of the least massive galaxy on the most radial orbits, the relation is followed closely.
Also, when there is no stripping, the snapshots lay within the scatter of the data.
When there is stripping (for the simulation ID~41, only for the orbit on $50$~kpc pericenter) the snapshots move along the relation.
...
It is possible to compute the radial profile of $\lambda_R(r)$ including in the summation of eq. \ref{eq:lambda_r} only those pixels that have a galactocentric distance smaller than $r$.
...
\citet{Emsellem2007} in their Appendix A, present a description of three main effects at play when trying to recover the apparent angular momentum.
In their formal exposition, they describe the approximations made to relate $\lambda_R$ to the spin parameter $\lambda$ \citep{BinneyTremaine2008}:
\begin{equation}
  \lambda = \frac{J\sqrt{|E|}}{G M^{2.5}} = \frac{j\sqrt{|E|/M}}{G M},
\end{equation}
where $J = jM$ is the total angular momentum, $M$ the galaxy of the mass and $E$ the total energy of the system.
The approximations depend on the projection, luminous distribution and mass distribution of the galaxy.
