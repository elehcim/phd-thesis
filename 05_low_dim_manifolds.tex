...
\begin{figure}
  ...
  \caption{Schematic of the partition of the space around the manifold in two concentric shells of uniformly distributed sample points $\vect{p}_i$ constituted by segmented cylinders (only one is shown on the left) build from one center to another of the manifold $\cM$ (in black).}
  \label{fig:cylinder}
\end{figure}
...
%\paragraph{Scaling}
The scaling operator is the diagonal matrix $S = \tr(1, 1,  d_{\ell})$.
%\[
%S =
%\begin{pmatrix}
%  1 & 0 & 0\\
...
\begin{equation}
  \overline{V}_{i} = \langle V(d^r_{i-1},d^r_i)\rangle = \frac 1 N \sum_{j\in\cI_i} \langle V_m(p'_j)\rangle,
\end{equation}
obtaining the mean of $V$ over the cylindrical shell between $()d^r_{i-1},d^r_i)$ for every $i=1,\dots,N^r$.
