...
%lies in the ($d-1$)-dimensional simplex $\cS_0$  with vertices $\s_1 = (1, 0, 0, \dots ,0)$, $\s_2 = (1/2, 1/2, 0, \dots, 0)$, $\s_3 = (1/3, 1/3, 1/3, \dots, 0)$, \dots $\s_d = (1/d, 1/d, \dots ,1/d)$. The simplex 
%$\cS_0$ is a subset of the standard simplex $\cS$ with vertices equal to the standard basis
%$\e_1 = (1, 0, 0, \dots, 0); \e_2 = (0, 1, 0, \dots, 0); \e_d = (0, 0, \dots, 1)$.
%Considering $\cS$ as the simplex of multinomial distributions, the appropriate Riemannian geodesic distance is the Fisher distance \cite{Lebanon:2005:RGS:1087529}. In particular, for any $\tilde{\Lambda}_k,\tilde{\Lambda}_l \in \cS$, 
...
%Note that the vertex $\s_j$ of $\cS_0$ corresponds to the ideal normalized eigen-spectrum of a $j$-dimensional neighbourhood. Hence, we will quantify the degree to which the neighbourhood of $\tilde{\vect{t}}_i$ resembles a $j$-dimensional hyperplane in $\RR^d$ by the closeness of $\tilde{\Lambda}_i$ to $\s_j$
