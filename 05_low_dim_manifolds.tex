...
%\end{equation}
\begin{equation}\label{eq:WeightedVar}
  \overline{\mathcal{I}}^{\text{\cii{}}}_j = \frac{\sum_{i=1}^{N} \mathcal{I}^{\text{\cii{}}}_i W (r_i - \bar{\vect{v}}_j, h_i) m_i/\rho_i }{\sum_{k=1}^{N} W (r_i - \bar{\vect{v}}_j, h_i) m_i/ \rho_i}
\end{equation}
...
From each pair of adjacent centers we can compute the tangent bundle on the 1D manifold:
\begin{equation}
...
To do so, let us now consider a point cloud $\cC$ containing points $\vect{p}$, uniformly sampling a cylindrical volume of radius $1$, aligned along the $z-$axis and centered at the origin $\cO$.
\begin{figure}
  \centering
  \includegraphics[width=0.6\textwidth]{CylinderAnnotated.pdf}
  \caption{Schematic of the partition of the space around the manifold in concentric shells of uniformly distributed sample points $\vect{p}_i$ constituted by segmented cylinders build from one center to another (in black)}
  \label{fig:cylinder}
\end{figure}
...
\footnote{\textbf{Scaling:} In matrix notation, the scaling operator is:
\[
S =
\begin{pmatrix}
  1 & 0 & 0\\
...
\]
\textbf{Rotation}
We can compute the quaternion $\quat{q}$ to rotate the $z$ versor $\hat{\vect{k}}$ into $\hat{\vect{v}}_{\ell}$. From equation \eqref{eq:quaternion_from_two_versors}
...
%\end{pmatrix}
%\]
\textbf{Shift} We can then shift the scaled and rotated point cloud so that its origin is on center $\ti_{\ell}$.\\
...
\begin{equation}\label{eq:WeightedMean}
  \langle V(\vect{p}') \rangle = \dfrac{\sum\limits_{\vect{t}_j \in \vect{Q}} \dfrac{m_j}{\rho_j} V(\vect{t}_j) W(q_j,h_j)}{\sum\limits_{i =1}^{|\vect{Q}|} \dfrac{m_j}{\rho_j} W(q_i,h_i)}
\end{equation}
...
Computing $\langle V(\vect{p}')\rangle$ for every $\vect{p}' \in \cC'$ we can now evaluate the mean value of $V$ over the concentric cylindrical shells built in the original manifold defined by the index families $\cI_1,\dots,\cI_{N^r}$ as:
