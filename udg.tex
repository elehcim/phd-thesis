...
\paragraph{Comparison with galaxy in the field}
...
We compute the specific angular momentum $\vect{j}_{se}$ of star particles within an effective radius from the center of the galaxy (densest stellar region):
\begin{equation}
...
We compare $\vect{j}_{se}$ with $\lambda_R$ in Figure \ref{fig:lambda_r_j_s}.
We notice that for example in simulation ID 69, $\lambda_R$ behaviour is very oscillating and quite prone to bursty star formation which can abruptly affect its value.
In our measurements, $\lambda_R$ fails to distinguish pericenter passages unequivocally.

In order to compare the proxy $\lambda_R$ with the measurable angular momentum, we try to retrieve an approximate formula for the specific angular momentum which relies on observable quantities:
\begin{equation}
 j_s^* = \lambda_R \cdot R_e \cdot \sigma_e 
\end{equation}
where $\sigma_e$ is the line-of-sight velocity dispersion of star particles measured within into an aperture.

For simplicity this 

The relation between $\lambda_R$ and the is shown in Figure~\ref{fig:lambda_r_j_s_sigma}
