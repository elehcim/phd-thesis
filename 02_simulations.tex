...
It can therefore emit photons when for example a free electron is captured by a ion of hydrogen.
Such photons, if the surrounding is not dense, can escape from the gas cloud, effectively removing energy from the gas.
Also, radiative energy losses can be due to other processes like bremsstrahlung, \ie{} when an electron is decelerated by other charges.
This implies a cooling of the gas with a decrease of temperature.
%Obviously the absorption of straying photons leads to heating the gas.
It is possible to compute the cooling rate of the gas depending on its ionization state and chemical constitution.
As described in the previous section, in simulations it is assumed that gas particles can be enriched in their metallicity content only in two ways: \mgfe{} and \feh{}.
They are tracers of the particle overall chemical composition.
We tabulate a small number of possible chemical compositions covering the range that can occur in a simulation \citep{DeRijcke2013}.
The cooling and heating rates can then be interpolated for each gas particle.
