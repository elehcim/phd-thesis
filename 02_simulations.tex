...
Computer simulations are essentially a tool to solve complex systems of equations which are intractable with analytic techniques, or only tractable with very coarse level of approximation \citep{Springel2015}.
...
On the other hand, as we shall see, gas is collisional and its behaviour at any point is affected by short-range interactions, whose modeling requires other assumptions (see below Sections  \ref{sec:dwarf_models} and \ref{sec:extended_gas_physics}).
...
\end{equation}
Usually a more general equation of state is assumed $p = (\gamma - 1) \rho \energy$, with $\gamma$ the adiabatic index.
...
It is interesting to note that in this description the molecules have basically completely vanished and have been replaced with a continuum fluid description.
...
The equatiosn of motion for particles, following equations \eqref{eq:poisson} can be written as follows:
\begin{equation}
 \vect a_i = -\nabla_i \Phi(\vect x_i),
\end{equation}
\begin{equation}
...
An estimate of this softening length is obtained by requiring that in the simulated gas clouds with the highest density, $\epsilon$ is about equal to the average distance between the gas particles \citep{Schroyen2013}. % Mail Sven 24/11/2017
...
The main idea of the SPH formalism is to write the equations of motion of the fluid for the sampling particles, solve them and then compute physical quantities in every point of the domain trough the so called \emph{SPH interpolation}.
...
An example of a kernel is a Gaussian:
\begin{equation}
...
% TODO h_i. Why each particle has its own smoothing length
...
\begin{multline}
  \tilde W  = \int W \left(\norm{\vect{r} - \vect{r}_i},h_i \right) \dd z' =\\
   \int W \left(\sqrt{(x-x_i)^2+(y-y_i)^2+(z-z_i)^2}, h_i \right) \dd z' %=\\
%    \tilde{W} \left(\sqrt{(x-x_i)^2+(y-y_i)^2}, h_i \right).
...
\end{equation}
We can then use a stochastic method to determine if an eligible gas particle has to be turned into a gas particle.
...
\end{equation}
where $c_s$ is the local sound speed $c_s=\sqrt{\gamma k_BT/\mean{\mu}}$ with $\mean{\mu}$ the mean molecular mass and $\rho$ and $T$ the local density and temperature respectively.

Following Jeans criterion, smaller scales are stable, but given that cooling by radiation scales quadratically with the density, while the pressure only scales linearly, the cloud will effectively become unstable.
...
Also, \citet{Bate1997} show that SPH, in problems involving self-gravity, produces numerical results which are only trustworthy so long as the minimum resolvable mass is always less than a Jeans mass (which is just the mass contained in a sphere of diameter $\lambda_J$).
...
A second type of feedback is the stellar wind from young O and B stars.
...
But this sudden acceleration removal can cause active particles during the numerical time integration to be kicked away unnaturally, because of the first term of the particles acceleration in equation \eqref{eq:mb_acceleration}.
...
We assume the cluster gas to have the density and pressure of the Fornax cluster as described by \citet{Paolillo2002} and in \refsec{sec:ICM}.
...
\begin{figure}
  \centering
  \includegraphics[width=\textwidth]{cartridge.pdf}
...
At each timestep $\Delta t$ the correct amount of new particles $N_c^{\mathrm{new}}$ needed to fill the volume $V = L^2\, h = L^2 \norm{\Vp}\Delta t$ in the moving box, see Figure~\ref{fig:cartridge}, can be computed as:
\begin{equation}
 N_c^{\mathrm{new}} = \dfrac{\rho_c(r) V}{m_g}.
 \label{eq:nc_new}
\end{equation}
%Similarly, the density of the glass $\rho_{\text{glass}}$ and of the cluster  can be computed with the relation:
%\begin{equation}
%  \rho_{\text{glass}} = \dfrac{N_\text{glass}\,l^3}{m_g}.
%\end{equation}
...
\begin{equation}
  V' = \frac{l^3 h}{L} \frac{N_c}{N_{\text{glass}}}.
\end{equation}
...
% In general for a massively parallel code, it is not optimal to have to rearrange because of cache misses and interruption to the flow of the parallel processors because of additional message passing.
% The arrays representing the particles undergo an optimized kind of algorithm.
